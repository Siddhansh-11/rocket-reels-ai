...
class ScriptingAgent:
    ...
    def generate_script(self, input_data: Dict) -> str:
        ...
        # Step 2: Generate hook first
        hook = self.hook_generator.create_hook(content_type, input_data.get('topic', input_data.get('title', '')))
        
        # Step 3: Add hook to input data for template population
        input_data_with_hook = input_data.copy()
        input_data_with_hook['hook'] = hook
        
        # Step 4: Select and populate template with hook included
        template_data = self.selector.select_template(content_type, input_data_with_hook)
